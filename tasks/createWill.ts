...
task('createWill', async () => {
  ...
  if (PK != null) {
    ...
    const data = iface.encodeFunctionData('saveWillHash', [[will, xChainWill]])
    ...
    const safeEnableModuleTransactionData: SafeTransactionDataPartial = {
      to: ethers.utils.getAddress(sepoliaSafe),
      value: '0',
      data: enableModuleData
    }

    const enableTransaction: SafeTransaction = await safeSdk.createTransaction({ safeTransactionData: safeEnableModuleTransactionData })

    const enableTxResponse = await safeSdk.executeTransaction(enableTransaction)
    await enableTxResponse.transactionResponse?.wait()
    console.log('enable')
    /*
    const execData = iface.encodeFunctionData('execute', [will, xChainWill])
    const safeExecTransactionData: SafeTransactionDataPartial = {
      to: baseGMmAddress,
      value: '0',
      data: execData

    }
    const safeExecTransaction = await safeSdk.createTransaction({ safeTransactionData: safeExecTransactionData })
    const executeExecTxResponse = await safeSdk.executeTransaction(safeExecTransaction)
    console.log(executeExecTxResponse.hash)
    await executeExecTxResponse.transactionResponse?.wait() */
  }
})
